...
class VocabularyEntry(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)  # Updated to user_id
    ...
    selected = db.Column(db.String(3), default='no')
    used_as_distractor = db.Column(db.String(3), default='no')

    user = db.relationship('User', backref=db.backref('vocabulary_entries', lazy=True))  # Relationship to User
...
class ContextEntry(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('context_entries', lazy=True))  # Relationship to User
