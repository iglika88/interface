...
def find_target_word(context, vocabulary_item):
    ...
    # Return None if words list is empty to handle the case of no words in context
    if not words:
        return None
...
def generate_and_validate_contexts(item, pos, domain, level):
    ...
    model_switch_counter = 0  # Counter to ensure models switch on failure
    max_attempts = 100  # Set a cap to avoid infinite loops

    while len(contexts) < 5 and attempts_counter < max_attempts:  # Ensure 5 valid contexts are generated
        model_name = models[model_switch_counter % len(models)]  # Switch between models
        ...
        try:
            ...
            # Capitalize the target word if it's the first word in the context
            if context.lower().startswith(item.lower()):
                context = item.capitalize() + context[len(item):]
            ...
            if is_valid and context:
                ...
                if linguistic_checks_enabled:
                    ...
                    if context:
                        print(f"Generated with {generation_method}: {context}")
            ...
            # Switch to the next model on either failure or success
            model_switch_counter += 1

            # Break loop if 5 contexts are generated or max attempts are reached
            if len(contexts) >= 5 or attempts_counter >= max_attempts:
                break

        except Exception as e:
            print(f"Error generating context with {model_name}: {e}")
            model_switch_counter += 1  # Switch to the next model on exception
            continue

    # Ensure exactly 5 contexts are generated
    cleaned_contexts = clean_text([c[0] for c in contexts])
    return [(cleaned_contexts[i], contexts[i][1]) for i in range(len(contexts))]




def generate_contexts_no_validation(item, pos, domain, level):
    contexts = []
    lower = 8 if level == 'B1' else 17
    upper = 43 if level == 'B1' else 87
    ...
    models = ['gemini', 'mistral']
    model_switch_counter = 0  # Counter to alternate models
